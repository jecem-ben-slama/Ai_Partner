...
class StorageService {
  ...
  /// Modified: Now uses the ID from the result if available to stay in sync with UI.
  Future<void> saveScan(List<Map<String, dynamic>> resultsJson) async {
    final prefs = await SharedPreferences.getInstance();
    List<String> history = prefs.getStringList(_historyKey) ?? [];

    // Check if the first result has an ID generated by the VisionResult model
    final String? resultId = resultsJson.isNotEmpty
        ? resultsJson[0]['id']
        : null;

    final entry = {
      // Use the existing ID from the scan result, or fallback to timestamp if missing
      'id': resultId ?? DateTime.now().millisecondsSinceEpoch.toString(),
  ...
  /// Overwrites the entire history list.
